...
# Mapa de CNPJs da empresa → nome da unidade
# Substitua pelos CNPJs e nomes reais da sua empresa
CNPJS_EMPRESA = {
...
def identificar_unidade(cnpj):
    return CNPJS_EMPRESA.get(limpar_cnpj(cnpj), "")

def is_empresa(cnpj):
    return limpar_cnpj(cnpj) in CNPJS_EMPRESA

def derivar_papel(cnpj_prestador: str, cnpj_tomador: str,
    ...
    eh_prest  = prest  in CNPJS_EMPRESA
    eh_tomad  = tomad  in CNPJS_EMPRESA
    eh_interm = interm in CNPJS_EMPRESA
...
def parse_nfse(path):
    ...
    vbc   = to_float(txt(vals_inf, "vBC"))
    paliq = to_float(txt(vals_inf, "pAliqAplic"))
    viss  = to_float(txt(vals_inf, "vISSQN"))
    vliq  = to_float(txt(vals_inf, "vLiq"))
    ...
    aliq_iss_dps = to_float(txt(trib_mun_el, "pAliq"))
    ...
    chave_nfse = (txt(inf, "chNFSe") or inf.get("Id", "")).lstrip("NFS")
    ...
    papel         = "Tomadora"      if is_empresa(cnpj_toma)   else \
                    "Prestadora"    if is_empresa(cnpj_emit)   else \
                    "Intermediária" if is_empresa(cnpj_interm) else "Outro"
    papel_empresa = derivar_papel(cnpj_emit, cnpj_toma, cnpj_interm)
...
def gerar_excel(pasta_xmls="xmls", pasta_raiz="excel_competencias"):
    ...
    print(f"Combinações unidade/competência: {len(todas_chaves)}\n")
...
def gerar_csv_consolidado(pasta_xmls="xmls",
                          saida_nfse="dados/nfse_consolidado.csv",
                          saida_eventos="dados/eventos_consolidado.csv"):
